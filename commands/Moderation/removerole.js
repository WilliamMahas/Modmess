...
const db = require('quick.db');
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.hasPermission("MANAGE_ROLES")) return message.channel.send("**You don't have the right permissions to use this command! - [MANAGE_ROLES]**");

        if (!message.guild.me.hasPermission("MANAGE_ROLES")) return message.channel.send("**I don't have the right permissions to remove roles from users! - [MANAGE_ROLES]**");
        ...
        if (rMember.roles.highest.comparePositionTo(message.guild.me.roles.highest) >= 0) return message.channel.send('**I cannot remove the role from this user! - [Higher Than Me In Role Hierachy]**')
        if (message.guild.me.roles.highest.comparePositionTo(role) < 0) return message.channel.send('**Role is currently higher than mine, therefore i cannot remove it!**')
        if (role.managed) return message.channel.send("**Cannot remove that role from this user!**")

        if (!rMember.roles.cache.has(role.id)) return message.channel.send("**User doesn't have the role**")
