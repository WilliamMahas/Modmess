...
const db = require('quick.db');
    ...
    run: async (bot, message, args) => {
        try {
            if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("**You don't have permissiosn to kick users! - [KICK_MEMBERS]**");
            if (!message.guild.me.hasPermission("KICK_MEMBERS")) return message.channel.send("**I don't have permissions to kick users! - [KICK_MEMBERS]**");
            ...
            if (!kickMember) return message.channel.send("**User is not in this guild!**");

            if (kickMember.id === message.member.id) return message.channel.send("**You can't kick yourself**")
